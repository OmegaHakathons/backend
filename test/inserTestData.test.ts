...
describe.skip('(e2e)', () => {
    ...
    const handle = async <T extends ObjectLiteral>(type: EntityClassOrSchema, number: number, getOne: () =>  T) => {
    ...
    it("insert test data", async () => {
        const cars = await handle<Car>(Car, 10, () => ({
            id: randomInt(100000),
        ...
        const aggregates = await handle<Aggregate>(Aggregate, 10, () => ({
            id: randomInt(100000),
